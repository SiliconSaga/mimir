...
The `pgbouncer.get_auth()` lookup that query calls is usually **already there**: Percona's PostgreSQL operator installs it into `template1`, so every database `CREATE DATABASE` makes inherits the schema, the function and its grants. An existing admin-owned function is left exactly as found — theirs is stricter than ours, refusing to return a verifier for a superuser, a replication role, or the pooler itself, and it sits on the client login path where churn is not free. A fallback is written only when none exists, or when the *tenant* created one (they own the database, so they can), in which case it is dropped rather than replaced.
